...
HYPRTRAILS_CONF="$HOME/.config/hypr/custom/env.conf"
DEFAULT_HEX="#00ffff"
NEW_HEX=""
TRAIL_ALPHA="ff"
...
update_hyprtrails() {
    if [[ -f "$HYPRTRAILS_CONF" ]]; then
        ...
        local HEX="${NEW_HEX//#/}"
        local FINAL_HEX="${HEX}${TRAIL_ALPHA}"

        awk -v new_color="        color = rgba(${FINAL_HEX})" '
            BEGIN {in_plugin=0; in_trails=0}
            /^\s*plugin\s*{\s*$/ {in_plugin=1}
            in_plugin && /^\s*hyprtrails\s*{\s*$/ {in_trails=1}
            in_plugin && in_trails && /^\s*color\s*=\s*rgba\(.*\)\s*;.*$/ {
                print new_color
                next
            }
            /^\s*}\s*$/ {
                if (in_plugin && in_trails) {
                    in_plugin=0
                    in_trails=0
                } else if (in_plugin) {
                    in_plugin=0
            ...
            { print }
        ' "$HYPRTRAILS_CONF" > "${HYPRTRAILS_CONF}.tmp" && mv "${HYPRTRAILS_CONF}.tmp" "$HYPRTRAILS_CONF"
    else
